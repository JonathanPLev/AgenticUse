// enhanced_instrumentation_final_fix.js
// FINAL FIX: Robust instrumentation with proper function name recording and frame handling

const { DataQueue } = require('./helpers');

/**
 * FINAL FIX: Enhanced page instrumentation with proper function name recording
 */
async function enhancedInstrumentPage(page, queues) {
  ...
  // FINAL FIX: Enhanced debugger configuration for proper function name recording
  try {
  ...
  const MAX_FRAME_LOGS = 2; // Further reduced
  ...
  // FINAL FIX: Enhanced debugger event handling with comprehensive function name capture
  ...
  client.on('Debugger.paused', async (params) => {
    try {
      ...
      // FINAL FIX: Enhanced call frame processing with comprehensive function name extraction
      const enhancedCallFrames = await Promise.all(callFrames.map(async (frame) => {
        ...
        if (!functionName || functionName === '') {
          try {
            ...
            // Method 2: Use scope information
            if (!functionName && frame.scopeChain && frame.scopeChain.length > 0) {
              try {
                ...
                if (localScope && localScope.object && localScope.object.objectId) {
                  ...
                  if (properties && properties.result) {
                    const funcProp = properties.result.find(prop => 
                      prop.name === 'arguments' || prop.name === 'callee'
                    );
                    if (funcProp && funcProp.value && funcProp.value.description) {
                      const match = funcProp.value.description.match(/function\s+(\w+)/);
                      if (match && match[1]) {
                        functionName = match[1];
                      }
                    }
            ...
            // Method 3: Fallback to URL-based naming
...
/**
 * FINAL FIX: Process content with better error handling for detached frames
 */
